<!-- <template>
  <div class="border border-gray-500 rounded px-10">Reach throught me @gathogo</div>
  <div class="gallery">
...
<template>
  ...
  <div class="relative scale-115 mb-10 md:scale-100 min-h-screen overflow-hidden  p-0">
    ...
    <div class="absolute inset-0 overflow-hidden">
      ...
      <!-- Floating Particles -->
      <div v-for="(particle, index) in particles" :key="index" class="absolute rounded-full pointer-events-none" :style="{
    ...
    <div class="absolute top-2 left-0 right-0 flex justify-between items-center px-6 z-10">
      ...
      <div class="flex space-x-2">
        ...
        <i class="fas fa-battery-full text-white text-xs"></i>
      </div>
    </div> -->
        ...
        <div class="relative pt-12 pb-8 px-7 h-full">
          ...
          <div class="text-center mt-8 mb-8">
            <h1 class="text-3xl md:text-4xl font-bold mb-4" v-animate-typing="{
              ...
              <span
                class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-indigo-500 animate-text-shine">
                Connect With Me
              </span>
            </h1>
          ...
          <div
            ...
            <div
              class="absolute -bottom-1/3 -left-1/3 w-full h-full bg-purple-500/10 rounded-full filter blur-3xl animate-glow-pulse opacity-0 group-hover:opacity-100 transition-opacity duration-1000"
              style="animation-delay: 2s;"></div>

            <form action="https://formsubmit.co/kariuki12nicholas@gmail.com" method="POST" @submit="handleSubmit"
              ...
              <div class="relative group">
                <input v-model="form.name" type="text" id="name" name="name" required
                  class="w-full bg-gray-700/40 border-0 border-b border-gray-600/50 py-3 px-2 text-white placeholder-gray-400 focus:outline-none focus:ring-0 peer transition-all duration-300 rounded-t"
                  placeholder="Name e.g achiever" />
                <label for="name" hidden
                  class="absolute left-2 top-2 text-gray-400 text-sm transition-all duration-300 peer-focus:-top-5 peer-focus:text-xs peer-focus:text-blue-400">
                  Your Name
                </label>
                <div
                  class="absolute left-0 bottom-0 w-0 h-0.5 bg-gradient-to-r from-blue-500 to-purple-600 transition-all duration-500 peer-focus:w-full">
                </div>
              </div>
              ...
              <!-- Message Field -->
              <div class="relative group">
                <textarea v-model="form.message" id="message" name="message" rows="4" maxlength="500" required
                  class="w-full bg-gray-700/40 border-0 border-b border-gray-600/50 py-3 px-2 text-white placeholder-gray-400 focus:outline-none focus:ring-0 peer transition-all duration-300 rounded-t resize-none"
                ...
                <div class="text-xs text-gray-400/80 mt-1 text-right">{{ form.message.length }}/500</div>
              </div>

              <!-- Submit Button -->
              <button type="submit"
                class="relative overflow-hidden group px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg font-medium text-white transition-all duration-500 w-full"
                :disabled="isSubmitting">
                <span class="relative z-10 flex items-center justify-center gap-2">
                  <i class="fas fa-paper-plane text-sm transition-transform duration-300 group-hover:translate-x-1"
                    :class="{ 'rotate-45 scale-125': isSubmitting }"></i>
                  <span class="text-sm transition-all duration-300 group-hover:translate-x-1">
          ...
          <!-- Social Links -->
          <div class="flex justify-center mt-8 md:mt-30 space-x-4">
            <a v-for="(social, index) in socialLinks" :key="index" :href="social.url" target="_blank"
              class="social-icon relative w-12 h-12 rounded-xl bg-gray-800/50 flex items-center justify-center text-xl transition-all duration-500 hover:-translate-y-1 group">
              <div
                class="absolute inset-0 rounded-xl bg-gradient-to-br opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                :style="{ background: `linear-gradient(135deg, ${social.color} 0%, ${social.secondaryColor || social.color} 100%)` }">
              </div>
              <i :class="social.icon" class="relative z-10 transition-all duration-300"
                :style="{ color: hoveredSocial === index ? 'white' : social.color }"></i>
            </a>
          </div>
        </div>
    ...
    <transition name="confetti">
      <div v-if="showConfetti" class="fixed inset-0 pointer-events-none z-50">
        <div v-for="(confetti, index) in confettiCount" :key="index" class="absolute rounded-sm opacity-90" :style="{
          ...
          animation: `confetti-fall-3d ${Math.random() * 3 + 2}s ease-in forwards`,
    ...
    <transition name="modal-3d">
      <div v-if="showSuccessModal"
        ...
        <div
          class="relative bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-8 sm:p-10 max-w-md mx-4 text-center border border-gray-700/50 shadow-2xl transform transition-all duration-500"
          :style="{
            transform: modalHover ? 'perspective(1000px) rotateX(5deg) rotateY(5deg) scale(1.03)' : 'perspective(1000px) rotateX(0) rotateY(0) scale(1)',
            boxShadow: modalHover ? '0 25px 50px -12px rgba(99, 102, 241, 0.3)' : '0 20px 25px -5px rgba(0, 0, 0, 0.1)'
          }" @mouseenter="modalHover = true" @mouseleave="modalHover = false">
          <div class="absolute -top-5 -right-5 w-16 h-16 bg-blue-500/10 rounded-full filter blur-xl animate-pulse">
          </div>
          <div class="absolute -bottom-5 -left-5 w-16 h-16 bg-purple-500/10 rounded-full filter blur-xl animate-pulse"
            style="animation-delay: 1.5s;"></div>

          <div class="text-green-400 text-6xl sm:text-7xl mb-6 transform transition-all duration-500" :style="{
            transform: modalHover ? 'scale(1.1) rotate(5deg)' : 'scale(1) rotate(0)',
            filter: modalHover ? 'drop-shadow(0 0 10px rgba(74, 222, 128, 0.5))' : 'none'
          }">
            <i class="fas fa-check-circle"></i>
          </div>
          <h3 class="text-2xl sm:text-3xl font-bold text-white mb-3">Message Sent Successfully!</h3>
          <p class="text-gray-300 mb-6 text-sm sm:text-base leading-relaxed">
            Thank you for reaching out! I've received your message and will get back to you within 24 hours.
            A confirmation has been sent to your email address.
          </p>
          <button @click="showSuccessModal = false"
            class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg text-white transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-blue-500/20">
            Close Window
          </button>
        </div>
      </div>
...
const createParticles = () => {
  ...
  for (let i = 0; i < particleCount; i++) {
    particles.value.push({
      ...
      size: Math.random() * (window.innerWidth < 640 ? 20 : 40) + 10,
...
const messages = [
  ...
  "Innovation begins with a simple hello."
]
...
const confettiCount = ref(200)

// Auto-response message
const autoResponseMessage = computed(() => {
  return `Hi ${form.name || 'there'},\n\nThank you for reaching out! I've received your message and will get back to you within 24 hours.\n\nBest regards,\nNicholas Kariuki`
})
...
const focusField = (field) => {
  document.getElementById(field).parentElement.classList.add('!border-indigo-400/70')
}

const blurField = (field) => {
  document.getElementById(field).parentElement.classList.remove('!border-indigo-400/70')
}

const handleSubmit = (e) => {
  // Validate form fields
  if (!form.name.trim()) {
    shakeField('name', "Please enter your name")
    e.preventDefault()
    return
  }

  if (!form.email.trim()) {
    shakeField('email', "Please enter your email address")
    e.preventDefault()
    return
  }
  ...
  if (!form.message.trim()) {
    shakeField('message', "Please enter your message")
    e.preventDefault()
    return
  }
  ...
  // Show success UI (FormSubmit will handle actual submission)
  setTimeout(() => {
...
const shakeField = (fieldId, message) => {
  const field = document.getElementById(fieldId)
  field.parentElement.classList.add('animate-shake')
  setTimeout(() => {
    field.parentElement.classList.remove('animate-shake')
  }, 1000)
}
...
const socialLinks = [
  ...
  },
  {
    ...
    url: 'https://linkedin.com/in/gathogo',
    color: '#9ca3af',
...
const confettiColors = [
  '#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#f59e0b', '#10b981', '#6366f1', '#a855f7'
]
...
@keyframes confetti-fall-3d {
  0% {
    transform: translate3d(0, -100vh, 0) rotateX(0) rotateY(0) rotateZ(0);
    opacity: 1;
  }

  100% {
    transform: translate3d(calc(var(--random-x) * 100px), 100vh, calc(var(--random-z) * 100px)) rotateX(calc(var(--random-rotate) * 360deg)) rotateY(calc(var(--random-rotate) * 360deg)) rotateZ(calc(var(--random-rotate) * 360deg));
    opacity: 0;
  }
}

/* Modal 3D transitions */
